    ...
    return (
      <View style={{ alignItems: 'flex-start', marginTop: 5, marginRight: 40, marginBottom: 5 }}>
        <View style={{ backgroundColor: '#bfbfbf', borderRadius: 10 }}>
          <Text style={{ fontSize: 16, color: '#0d0d0d', padding: 8, elevation: 1 }}>
            {text.message}
          </Text>
          <Text style={{ fontSize: 12, color: '#999999', marginLeft: 10, marginBottom: 5 }}>
            {text.created}
          </Text>
        </View>
      </View>
      ...
      <View style={{ flex: 1, marginTop: 5, backgroundColor: '#eee4dc', padding: 10 }}>
        ...
        <View style={{ flexDirection: 'row', height: 60 }}>
          ...
          <TouchableHighlight onPress={this._sendMessage.bind(this)} underlayColor="#fff">
            <Image
              source={require('../images/ic_button_send_sms.png')}
...
const mapStateToProps = state => {
  return {
    conversation: state.ListConversation.data,
    message: state.AppReducer.message,
    authUsername: state.AuthReducer.authUsername,
  };
};
